...
from tbv_functions import wait_for_data, calc_signal_var, get_mean_roi, parse_prt_file, calc_correlation
from midi_functions import create_chord_from_number, send_chord_on, send_chord_off, clear_all_notes
...
# Select PRT path
prtPath = '/Users/alexandresayal/GitHub/phd-main-exp/prt_creator/prt/MusicNF_Main_v0.3_PostDiscard.prt'
...
while time_counter < expectedTime:
    ...
    if time_counter == currentTime:
        ...
        SignalVar[time_counter], SignalVarDiscrete[time_counter] = calc_signal_var(Baseline[time_counter], ROImeans[time_counter, selected_roi], maxPSC)
        ...
        if (current_condition == upregulation_index):
            ...
            # Get Chord
            if FEEDBACK:
                chord, previous_chord, CHANGE_CHORD = create_chord_from_number(PearsonCorrDiscrete[time_counter, 0], PearsonCorrDiscrete[time_counter-1, 0], chord, imagery_counter)
            else:
                chord, previous_chord, CHANGE_CHORD = create_chord_from_number(PearsonCorrDiscreteSham[time_counter, 0], PearsonCorrDiscreteSham[time_counter-1, 0], chord, imagery_counter)
...
if FEEDBACK:
    ...
    np.savez(f'MusicNF_{f_name}_{timestamp}.npz', 
         ROImeans=ROImeans, 
