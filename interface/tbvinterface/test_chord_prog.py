...
for ii in range(1, len(base_note_prog)):
    chord, previous_chord, CHANGE_CHORD = create_chord_from_number(base_note_prog[ii], base_note_prog[ii - 1], chord)
    if CHANGE_CHORD:
        send_chord_off(midi_music_out, previous_chord)
        send_chord_on(midi_music_out, chord)
    if ii == 10:
        send_chord_on(midi_beep_out, [60])
    if ii == 11:
        send_chord_off(midi_beep_out, [60])
